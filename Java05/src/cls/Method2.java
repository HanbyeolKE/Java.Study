...
class MyClass5{
	// 메서드1 : 반환값 X, 입력값 X (오로지 코드 저장, 가독성)
		void 메서드1() {
			// 입력값이 없으면 ()를 비워놓습니다.
			// 반환값이 없으면 자료형은 void로 하고 return은 생략
			// return;
			
			// 오로지 코드 재사용 (입력값 X, 반환값 X)
			int num = 1;
			System.out.println("나는 메서드" + num + "에 저장된 코드다");
			
		}
		
		// 메서드2 : 반환값 X, 입력값 O (코드 재활용, 일부만 변수인 경우)
		...
		// 메서드3 : 반환값 O, 입력값 X (코드 재사용, 메소드 사용 후에 전달해줄 값이 있음)
		String 메서드3() {
		...
		int 메서드4(double dnum) {
			...
			double result = dnum - (int)dnum;	// 소숫점 부분만 남겠죠?
			if(result >= 0.5) {
				dnum += 1.0;		// 만약에 소수점 부분이 0.5 보다 컸다면 + 1
			}
